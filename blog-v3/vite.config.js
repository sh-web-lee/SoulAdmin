...
export default defineConfig(() => {
  const { VITE_HTTP_PROXY } = import.meta.env;

  const isEnableHttpProxy = VITE_HTTP_PROXY === 'Y';
  return {
    base: './',
    ...
    server: {
      ...
      proxy: isEnableHttpProxy ? {
        // 本地后端代理
        "/proxy-default": {
          //要访问的跨域的域名
          target: "http://localhost:8888/api",
          changeOrigin: true,
          rewrite: (path) => path.replace(new RegExp('^/proxy-default'), ""),
        },
        // minio 代理
        "/blog-images": {
