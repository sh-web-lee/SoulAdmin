...
const { upToAliyun, deleteObject } = require('../../utils/aliyunUpload')
...
class UtilsController {
    ...
    if (file) {
      if (UPLOADTYPE == "local" || !UPLOADTYPE) {
        ...
        if (UPLOADTYPE == "qiniu") {
          ...
          const res = await upToAliyun(file)
          if (res) {
  ...
  async deleteOss(ctx) {
    const res = await deleteObject(ctx)
    if (res) {
      ctx.body = result('success', null)
    } else {
      return ctx.app.emit("error", throwError(errorCodeUpload, "文件移除失败"));
    }
  }
