...
class ArticleService {
  ...
  async getArticleList(param) {
    ...
    const { current, size, article_title, is_top, status, tags:tag_id, category, createdAt } = param;
    ...
    promiseList = rows.map(async (v) => {
      let obj = {
        author: await getAuthorNameById(v.dataValues.author_id),
        category: await getCategoryNameById(v.dataValues.category_id),
        tags: await getTagListByArticleId(v.dataValues.id),
      };
    ...
    await Promise.all(promiseList).then((res) => {
      if (res.length) {
        rows.forEach((v, i) => {
          v.dataValues.author = res[i].author;
          v.dataValues.category = res[i].category;
          v.dataValues.tags = res[i].tags.tagNameList;
        });
      }
    });
